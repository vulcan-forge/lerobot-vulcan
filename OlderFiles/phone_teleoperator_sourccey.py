...
class PhoneTeleoperatorSourccey(Teleoperator):
    ...
    def _apply_mirroring(self, joints_deg: list[float]) -> list[float]:
        """Mirror joint angles around configured neutral offsets if enabled.
        joints_deg follows the action_features order: [shoulder_pan, shoulder_lift, elbow_flex, wrist_flex, wrist_roll, gripper].
        """
        if not getattr(self.config, "mirror_enabled", False):
            return joints_deg
        values = list(joints_deg)
        neutrals = list(getattr(self.config, "mirror_neutral_deg", (0.0,) * len(values)))
        out = values[:]
        for idx in range(min(5, len(values))):
            out[idx] = 2.0 * neutrals[idx] - values[idx]
        return out
