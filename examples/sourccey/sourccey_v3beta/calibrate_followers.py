...
from lerobot.robots.sourccey.sourccey_v3beta.sourccey_v3beta_follower.config_sourccey_v3beta_follower import (
    SourcceyV3BetaFollowerConfig,
)
from lerobot.robots.sourccey.sourccey_v3beta.sourccey_v3beta_follower.sourccey_v3beta_follower import (
    SourcceyV3BetaFollower,
)


"""
Calibrate Sourccey V3 Beta follower arms (manual calibration).
...
# Calibrate LEFT only
python examples/sourccey/sourccey_v3beta/calibrate_followers.py \
  --arms left \
  --left-port COM28 \
...
def _calibrate_arm(side: str, port: str, device_id: str) -> None:
    ...
    cfg = SourcceyV3BetaFollowerConfig(
        port=port,
        id=device_id,
        orientation=orientation,
        cameras={},  # disable cameras for calibration-only flow
    )
    arm = SourcceyV3BetaFollower(cfg)
...
def main():
    parser = argparse.ArgumentParser(description="Manual calibration for Sourccey V3 Beta follower arms")
    ...
    parser.add_argument("--left-id", type=str, default="sourccey_v3beta_left", help="Calibration ID for left arm")
    parser.add_argument("--right-id", type=str, default="sourccey_v3beta_right", help="Calibration ID for right arm")
