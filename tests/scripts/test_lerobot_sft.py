...
from lerobot.scripts import lerobot_sft
...
""".strip()
    ...
    monkeypatch.setattr("sys.argv", ["lerobot_sft.py", f"--config_path={config_path}"])
    parser._config_path_args.clear()
    parser._config_yaml_overrides.clear()

    try:
        with patch.object(lerobot_sft, "run_training", return_value=None) as run_training:
            lerobot_sft.sft()
    finally:
        parser._config_path_args.clear()
        parser._config_yaml_overrides.clear()
