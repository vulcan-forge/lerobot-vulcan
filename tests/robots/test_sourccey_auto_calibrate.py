...
def test_z_controller_is_proportional_bounded_and_has_no_endpoint_boost(
    ...
    assert actuator.compute_command(position=0.0, target=2.0) == pytest.approx(0.5025)
    assert actuator.compute_command(position=0.0, target=-2.0) == pytest.approx(-0.5025)
    assert actuator.compute_command(position=0.0, target=100.0) == pytest.approx(1.0)
    assert actuator.compute_command(position=95.0, target=100.0) == pytest.approx(0.6075)
...
def test_z_controller_update_applies_motor_direction_inversion(
    ...
    assert position == 0.0
    assert driver.velocity_calls[-1] == ("linear_actuator", pytest.approx(-0.7825), True, True)
