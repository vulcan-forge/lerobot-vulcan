...
def test_from_env_falls_back_to_pairing_state_for_older_credentials(monkeypatch, tmp_path) -> None:
    credentials_path = tmp_path / "cloud_device_credentials.json"
    credentials_path.write_text(
        json.dumps(
            {
    ...
    credentials_path.with_name("cloud_pairing_state.json").write_text(
        json.dumps({"active_session_id": "session-5678"}),
        encoding="utf-8",
    )
