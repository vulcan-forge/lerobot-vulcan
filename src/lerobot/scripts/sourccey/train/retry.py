...
"""Retry wrapper shared by Sourccey training commands.

This script forwards all unknown CLI args to a selected Sourccey training wrapper.
...
def _extract_output_dir(train_args: list[str]) -> Path | None:
    ...
    if config_path is not None and config_path.is_file():
        try:
            ...
            logging.exception("Could not read output_dir from config file %s", config_path)
...
def _run_training_module(module: str, train_args: list[str]) -> int:
    # Run as a module so relative imports inside the training script resolve correctly.
    cmd = [sys.executable, "-m", module, *train_args]
    logging.info("Launching command: %s", " ".join(shlex.quote(part) for part in cmd))
    return subprocess.run(cmd, check=False).returncode


def _run_train(train_args: list[str]) -> int:
    """Launch the Sourccey pretraining wrapper."""
    return _run_training_module("lerobot.scripts.sourccey.train.pretrain", train_args)


def retry_main(
    training_module: str = "lerobot.scripts.sourccey.train.pretrain",
    training_name: str = "sourccey/train/pretrain.py",
) -> int:
    ...
    if output_dir is None:
        parser.error(
            "Missing output_dir. Provide --output_dir or set output_dir in a local YAML/JSON config."
        )
    ...
    for attempt in range(1, max_attempts + 1):
        ...
        exit_code = _run_training_module(training_module, retry_train_args)
