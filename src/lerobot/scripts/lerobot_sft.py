...
def run_training(
    ...
    preserve_checkpoint_normalization = (
        getattr(cfg.dataset, "normalization", None) == "checkpoint"
    )
    if preserve_checkpoint_normalization and getattr(cfg.policy, "type", None) == "groot":
        raise NotImplementedError(
            "Checkpoint normalization for SFT is not yet supported by the GROOT processor."
        )

    processor_kwargs = {}
    if (
        ((processor_pretrained_path and not cfg.resume) or not processor_pretrained_path)
        and not preserve_checkpoint_normalization
    ):
    ...
    if not cfg.is_reward_model_training and processor_pretrained_path is not None:
        ...
        if not preserve_checkpoint_normalization:
    ...
    if hasattr(dataset, "make_sampler"):
        ...
        if is_main_process:
            ...
            logging.info(
                "SFT dataset sampling weights: %s",
                {
                    name: round(weight, 6)
                    for name, weight in zip(
                        dataset.source_names, normalized_weights.tolist(), strict=True
                    )
                },
            )
    ...
    sft_source_counts = (
        torch.zeros(len(dataset.source_names), dtype=torch.int64, device=device)
        if is_main_process and hasattr(dataset, "source_names")
        else None
    )
    ...
    try:
        for _ in range(step, cfg.steps):
            start_time = time.perf_counter()
            batch = next(dl_iter)
            source_indices = batch.pop("sft_source_index", None)
            if sft_source_counts is not None and source_indices is not None:
                sft_source_counts += torch.bincount(
                    source_indices.to(device=device, dtype=torch.int64),
            ...
            if is_log_step:
                ...
                source_mix_metrics = {}
                if sft_source_counts is not None:
                    total_source_samples = int(sft_source_counts.sum().item())
                    if total_source_samples > 0:
                        source_mix_metrics = {
                            f"sft/source_fraction/{name}": count.item() / total_source_samples
                            for name, count in zip(
                                dataset.source_names, sft_source_counts, strict=True
                            )
                        }
                        logging.info("Observed SFT source mix: %s", source_mix_metrics)
                if wandb_logger:
                    wandb_log_dict = train_tracker.to_dict()
                    if output_dict:
                        wandb_log_dict.update(output_dict)
                    wandb_log_dict.update(source_mix_metrics)
...
def _prepare_padding_mask(policy: Any, batch: dict[str, Any], targets: torch.Tensor) -> torch.Tensor:
...
def _masked_xvla_forward(self: Any, batch: dict[str, Any]) -> tuple[torch.Tensor, dict]:
...
def _setup_sft_policy(policy: Any, cfg: SFTPipelineConfig) -> Any:
    """Install XVLA padded-action masking only when explicitly requested."""
