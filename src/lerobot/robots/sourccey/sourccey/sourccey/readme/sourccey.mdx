...
Relay agent autostarts with `sourccey_host` by default (control/data bridge only).

Required relay environment variables on robot host:
...
- `VULCAN_RELAY_AGENT_AUTOSTART=true|false` (default `true`)
