...
Default setup writes a `14.3V` pack charge target, `150mA` taper current,
and `10mA` quit current.
If your charger terminates differently, override these values:

```bash
uv run python src/lerobot/scripts/sourccey/battery/configure_bq34z100.py setup-4s-lifepo4 \
  --pack-charge-voltage-mv <actual_cv_mv> \
  --taper-current-ma <tail_current_ma> \
  --quit-current-ma <quit_current_ma>
```
...
If you want Sourccey to expose a dedicated 5th bottom camera as part of the
normal observation stream, enable it explicitly:

```
uv run -m lerobot.robots.sourccey.sourccey.sourccey.sourccey_host \
 --bottom_camera_enabled=true \
 --bottom_camera_path="/dev/v4l/by-path/platform-1000110000.pcie-pci-0001:01:00.0-usb-0:1.2.2:1.0-video-index0"
```

For SLAM-focused runs, you can disable the wrist cameras and have the robot host
publish the front-eye SLAM stream directly:

```
uv run -m lerobot.robots.sourccey.sourccey.sourccey.sourccey_host \
 --slam_eye_only_mode=true \
 --slam_eye_camera_fps=30 \
 --slam_eye_loop_freq_hz=30 \
 --slam_eye_width=320 \
 --slam_eye_height=240 \
 --slam_eye_fourcc=MJPG \
 --slam_eye_power_line_frequency=2 \
 --slam_eye_exposure_dynamic_framerate=true \
 --slam_eye_auto_exposure=3 \
 --slam_input_enabled=true \
 --slam_input_endpoint="tcp://*:5560" \
 --slam_publish_eye_only_mode=true \
 --slam_publish_fps=30 \
 --slam_resize_width=320 \
 --slam_resize_height=240
```

To include the dedicated bottom camera in that same SLAM packet while keeping the
front eyes as the stereo pair, add:
...
With `--bottom_camera_enabled=true`, the host now publishes `bottom` alongside
`front_left` and `front_right` on the SLAM `5560` stream.

Baseline recommendation:

- Keep the front-eye SLAM path at `320x240` unless you are explicitly running a resolution experiment.
- Treat `640x480` as experimental on Sourccey; it changes bandwidth, exposure behavior, and camera negotiation after reboot.

Useful experiment knobs:

- `--slam_eye_exposure_time_absolute=180` with `--slam_eye_auto_exposure=1`
- `--slam_eye_gain=12`
- `--slam_eye_sharpness=4`
- `--slam_eye_backlight_compensation=32`
- `--slam_eye_log_camera_warnings=true`
...
uv run lerobot-teleoperate \
 ...
 --teleop.left_arm_port="COM17" \
 --teleop.right_arm_port="COM18" \
 --fps=30 \
 --display_data=true \
 --robot.slam.input_enabled=false

```

To include the dedicated 5th bottom camera in the teleop observation stream and
Rerun display, add:
...
uv run lerobot-teleoperate \
 ...
 --fps=30 \
 --display_data=true \
 --robot.include_bottom_camera=true
```

When the bottom camera is enabled on the host, the client exposes it under the
observation key `bottom`.

For a SLAM-focused teleop run, raise the client loop to 60 FPS and publish only
the front stereo pair into the SLAM sidecar:
...
uv run lerobot-teleoperate \
 ...
 --fps=60 \
 --display_data=true \
 --robot.slam.input_enabled=true \
 --robot.slam.eye_only_mode=true
```

If SLAM publishing stalls teleop while the base is moving, throttle and shrink the
SLAM sidecar packets separately from the teleop loop:
...
uv run lerobot-teleoperate \
 ...
 --robot.slam.input_enabled=true \
 --robot.slam.eye_only_mode=true \
 --robot.slam.jpeg_quality=60 \
 --robot.slam.publish_fps=5 \
 --robot.slam.resize_width=160 \
 --robot.slam.resize_height=120
```
...
For a SLAM-focused Linux teleop run, use:
...
- `--dataset.single_task`: Task description for the dataset
...
uv run lerobot-rollout \
 --strategy.type=base \
 --policy.path="outputs/train/xvla_s_sourccey-shirt-fold-c-008/checkpoints/1000000/pretrained_model" \
 --robot.type=sourccey_client \
 --robot.id="sourccey" \
 --robot.remote_ip="192.168.1.214" \
 --task="Fold the shirt" \
...
uv run lerobot-rollout \
 ...
 --policy.path="outputs/train/xvla__sourccey-012__shorts-fold__cmb__005__mylesr/checkpoints/100000/pretrained_model" \
 --robot.type=sourccey_client \
 --robot.id="sourccey" \
 --robot.remote_ip="192.168.1.211" \
...
uv run lerobot-replay \
 --robot.type=sourccey_client \
 --robot.id="sourccey" \
 --robot.remote_ip="192.168.1.166" \
 --dataset.repo_id="sourccey-011/sourccey-011__set_table-a__set000__name" \
 --dataset.episode=0 \
 --dataset.fps=30
```
...
uv run lerobot-edit-dataset \
 --new_repo_id="Combination/sourccey-013**shirt-fold-dark-blue-a**set000-006**nickm" \
 --operation.type=merge \
 --operation.repo_ids="[
'sourccey-013/sourccey-013**shirt-fold-dark-blue-a/nickm/sourccey-013**shirt-fold-dark-blue-a**set000**nickm',
'sourccey-013/sourccey-013**shirt-fold-dark-blue-a/nickm/sourccey-013**shirt-fold-dark-blue-a**set001**nickm',
'sourccey-013/sourccey-013**shirt-fold-dark-blue-a/nickm/sourccey-013**shirt-fold-dark-blue-a**set002**nickm',
'sourccey-013/sourccey-013**shirt-fold-dark-blue-a/nickm/sourccey-013**shirt-fold-dark-blue-a**set003**nickm',
'sourccey-013/sourccey-013**shirt-fold-dark-blue-a/nickm/sourccey-013**shirt-fold-dark-blue-a**set004**nickm',
'sourccey-013/sourccey-013**shirt-fold-dark-blue-a/nickm/sourccey-013**shirt-fold-dark-blue-a**set005**nickm',
'sourccey-013/sourccey-013**shirt-fold-dark-blue-a/nickm/sourccey-013**shirt-fold-dark-blue-a**set006\_\_nickm'
]"
