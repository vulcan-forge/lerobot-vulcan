...
If the Unity VR app is already driving the robot through the normal Sourccey host connection, the cleanest recording setup is to let the Unity action stream carry a small recording-toggle bit to the host. The host surfaces that as a counter in robot observations, and `lerobot-record` on the client starts and stops episodes when that counter changes.
...
- pressing the wired recording button sends a `request_recording_toggle` bit on the normal action stream
- the host increments `recording_toggle_counter` on each new toggle request
- `lerobot-record` watches that counter through `sourccey_client` observations
- the first toggle starts an episode and the next toggle ends it
