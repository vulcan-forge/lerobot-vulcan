...
uv run src/lerobot/scripts/sourccey/auto_calibrate.py \
 --robot.type=sourccey \
 --robot.id=sourccey \
 --full_reset=True### Motor Configuration
```

Configure motor baud rate if needed:

```
uv run src/lerobot/scripts/sourccey/motor_config.py \
 --robot.type=sourccey \
 --robot.id=sourccey \
 --baud_rate=1000000### Teleoperator Calibration
```

Calibrate the bi-manual teleoperator (leader arms):
...
uv run src/lerobot/scripts/sourccey/auto_calibrate.py \
 ...
 --teleop.right_arm_port="COM8"For single-arm teleoperator:
```
...
python -m lerobot.calibrate \
 ...
 --teleop.port=COM28 \
 --teleop.id=sourccey_leader### Viewing/Editing Calibration Files
```
...
.venv\Scripts\activate # On WindowsThen run:
```
...
torchrun --nproc_per_node=2 src/lerobot/scripts/train.py \
 ...
 --num_gpus=2### Training on Pretrained Models
```
...
"sourccey-003/dataset-3"
 ...
 --output_path="sourccey-003/combined_dataset"**⚠️ Important:** Do not include trailing commas in the `--dataset_paths` array, as this will cause errors.
```
