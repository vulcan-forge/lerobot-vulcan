...
2. **Install LeRobot with Sourccey dependencies:**
   uv pip install -e ".[sourccey]"This installs the project in editable mode with all required extras for the Sourccey robot.
...
python src/lerobot/scripts/sourccey/auto_calibrate.py \
 ...
 --teleop.left_arm_port="COM14" \
 --teleop.right_arm_port="COM15"
```
...
python src/lerobot/control/sourccey/sourccey/record.py \
 ...
 --dataset.task="Lift your shoulder" \
 --dataset.fps=30 \
...
### Multi-GPU Training on xvla

accelerate launch \
  --multi_gpu \
  --num_processes=2 \
  $(which lerobot-train) \
  --dataset.repo_id=local/sourccey-012__shirtfold-1-a__cmb__chris \
  --policy.path="lerobot/xvla-base" \
  --policy.dtype=bfloat16 \
  --output_dir=outputs/train/xvla__sourccey-012__shirtfold-1-a__cmb__chris \
  --job_name=xvla__sourccey-012__shirtfold-1-a__cmb__chris \
  --steps=100000 \
  --save_freq=10000 \
  --batch_size=32 \
  --wandb.enable=false \
  --policy.push_to_hub=false \
  --rename_map='{"observation.images.front_left": "observation.images.image", "observation.images.front_right": "observation.images.image2", "observation.images.wrist_left": "observation.images.empty_camera_0"}'
