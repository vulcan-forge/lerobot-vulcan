...
class SourcceyFollower(Robot):
    ...
    def connect(self, calibrate: bool = True) -> None:
        ...
        # `auto_calibrate` connects with calibrate=False and must not be blocked by startup safety checks.
        self.configure(enforce_startup_safety=calibrate)
    ...
    def configure(self, *, enforce_startup_safety: bool = True) -> None:
        ...
        if startup_faults:
            details = "\n".join(
                f"  - {motor}: raw={raw}, range=[{range_min}, {range_max}]"
                for motor, (raw, range_min, range_max) in startup_faults.items()
            )
            raise RuntimeError(
