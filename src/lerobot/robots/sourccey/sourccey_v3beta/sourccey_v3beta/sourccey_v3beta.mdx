...
uv run -m lerobot.auto_calibrate --robot.type=sourccey --robot.id=sourccey_v3beta --arm=left
```
...
uv run -m lerobot.auto_calibrate --robot.type=sourccey --robot.id=sourccey_v3beta --full_reset=true --arm=left
```

# For manual calibration:
```bash
uv run -m lerobot.calibrate --robot.type=sourccey_v3beta --robot.id=sourccey_v3beta
```
...
python lerobot/scripts/combine_dataset.py \
    --repo_ids local/act__sourccey_v3beta-001__ai_test_6_shoulder-fast_chrism local/act__sourccey_v3beta-001__ai_test_6_shoulder-fast_chrism_2 \
...
# Normal default Lerobot Evaluate function.
  ...
  --teleop.right_arm_port=/dev/ttyACM2 \
  --teleop.id=bi_so100_leader \
  --dataset.repo_id=local/eval_ai_test_2 \
  --dataset.num_episodes=10 \
  --dataset.episode_time_s=25 \
  --dataset.reset_time_s=5 \
  --dataset.single_task="Grab the tape and put it in the cup" \
  --policy.path=outputs/train/act__ai_test_2/checkpoints/020000/pretrained_model \
  --dataset.push_to_hub=false \
  --display_data=false
