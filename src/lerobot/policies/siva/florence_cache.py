...
"""SIVA-owned disk cache for deterministic, frozen Florence context."""
...
class SIVAFlorenceFeatureCache:
    """Store SIVA's complete Florence boundary output by stable dataset index.

    The SQLite connection is lazy so checkpoint loading and policy serialization
    do not touch the filesystem. Rows are immutable for a cache signature, and
    ...
    def _connect(self) -> sqlite3.Connection:
        ...
        for key, value in expected.items():
            if metadata.get(key) != value:
                connection.close()
                raise ValueError(
                    f"SIVA Florence cache {self.path} has incompatible {key}: "
    ...
    def _serialize(
        ...
        auxiliary_visual_features: torch.Tensor,
        context_mask: torch.Tensor,
    ) -> bytes:
        return save_safetensors(
            {
                "vlm_features": vlm_features.detach().to(device="cpu").contiguous(),
                "auxiliary_visual_features": auxiliary_visual_features.detach()
                .to(device="cpu")
                .contiguous(),
                "context_mask": context_mask.detach().to(device="cpu").contiguous(),
            }
        )
    ...
    def get_many(self, sample_indices: list[int]) -> dict[int, dict[str, torch.Tensor]]:
        """Return cached contexts for requested indices, omitting cache misses."""
    ...
    def put_many(
        self,
        features: dict[int, tuple[torch.Tensor, torch.Tensor, torch.Tensor]],
    ) -> None:
        """Insert newly computed SIVA contexts without replacing existing rows."""
        if not features:
            return
        rows = [
            (index, self._serialize(vlm_features, auxiliary_visual_features, context_mask))
            for index, (vlm_features, auxiliary_visual_features, context_mask) in features.items()
        ]
