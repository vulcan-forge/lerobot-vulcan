...
def _read_word(bus: SMBus, cmd: int) -> int:
    # BQ34Z100 uses command pairs; LSB at cmd, MSB at cmd+1
    b = bus.read_i2c_block_data(BQ_ADDR, cmd, 2)
    return b[0] | (b[1] << 8)


def _read_sword(bus: SMBus, cmd: int) -> int:
    val = _read_word(bus, cmd)
...
def read_battery() -> BatteryStatus:
    with SMBus(I2C_BUS) as bus:
        ...
        remaining = _read_word(bus, CMD_REMAINING_CAP) * curr_scale
        full = _read_word(bus, CMD_FULL_CAP) * curr_scale
        voltage = _read_word(bus, CMD_VOLTAGE) * volt_scale
        avg_current = _read_sword(bus, CMD_AVG_CURRENT) * curr_scale
        temp_dK = _read_word(bus, CMD_TEMPERATURE)  # 0.1 K
